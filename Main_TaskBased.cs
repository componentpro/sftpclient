...
namespace SftpClient
{
    public partial class Main : Form
    {
        ...
        async void SftpConnect(Sftp sftp)
        {
            ...
            try
            {
                ...
                await client.ConnectAsync(_loginSettings.ServerName, _loginSettings.ServerPort); // SFTP connection
            }
            catch (Exception exc)
            {
                Util.ShowError(exc);
        ...
        async void SftpAuthenticate(Sftp sftp)
        {
            ...
            try
            {
                ...
                await sftp.AuthenticateAsync(_loginSettings.UserName, _loginSettings.Password, key);
            }
            catch (Exception ex)
            {
                Util.ShowError(ex);
        ...
        async void SftpShowItemProperties(ListViewItem item)
        {
            ...
            if (item.ImageIndex == _folderIconIndex && dlg.Recursive)
            {
                EnableProgress(true);
                    ...
                    try
                    {
                        await client.SetMultipleFilesAttributesAsync(dlg.FileName, attrs, dlg.Recursive, null);
                    }
                    catch (Exception ex)
                    {
